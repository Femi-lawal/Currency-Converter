...
getCurrencies.onload = function () {
    ...
    if (getCurrencies.status >= 200 && getCurrencies.status < 400) {
        for (const currency in data.results) {
            document.getElementById("convertFrom").innerHTML += `<option>${currency}</option>`;
            document.getElementById("convertTo").innerHTML += `<option>${currency}</option>`;
        }
    } else {
        console.log('error');
    }
